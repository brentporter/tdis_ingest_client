<template>
    ...
    <v-row class="ml-2 mr-2" no-gutters>
        <v-card
            ...
            <v-card-title>Geospatial Data Metadata Form</v-card-title>
            ...
            >
                ...
                >
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field
                                v-model="date"
                                label="Collections Publication Date"
                ...
                <v-text-field
                        v-model="time_period_content"
                        label="Time Period of Collection"
                        required
                        clearable
                        hint="What is the date span of the Content"
                ></v-text-field>
                <v-select
      ...
      :key="i" -->
        ...
        <v-card
            ...
            <v-card-text style="background-color: black;color:yellow;text-align: left;" class="pa-2">
                ...
                data_layer_title: <span style="color: antiquewhite">'{{title}}'</span><br/>
                description: <span style="color: antiquewhite">'{{description}}'</span><br/>
                collection: <span style="color: antiquewhite">'{{collectionAttached}}'</span><br/>
                ...
                time_period_of_content: <span style="color: antiquewhite">{{time_period_content}}</span> <br/>
                ...
                original_source: <span style="color: antiquewhite">{{source1}}</span> <br/>
                current_source: <span style="color: antiquewhite">{{source2}}</span> <br/>
    ...
    export default {
        ...
        methods: {
            invalidSize: function(){
                mapGeospatialDiv.invalidateSize();
                //console.log('Invalid SIZE');
            },
            resetMap: function(){
                mapGeospatialDiv.setView(this.center,5);
            },
            setupGeospatialLeafletMap: function () {
                mapGeospatialDiv = L.map("gMapContainer").setView(this.center, 5);
                ...
                esri.basemapLayer('Streets').addTo(mapGeospatialDiv);
            ...
            getMapExtent(){
                console.log(mapGeospatialDiv.getBounds()._southWest.lat);
                let extentCaptureWKT = [
                    ""+[mapGeospatialDiv.getBounds()._southWest.lat, mapGeospatialDiv.getBounds()._southWest.lng]+"",
                    ""+[mapGeospatialDiv.getBounds()._northEast.lat, mapGeospatialDiv.getBounds()._southWest.lng]+"",
                    ""+[mapGeospatialDiv.getBounds()._northEast.lat, mapGeospatialDiv.getBounds()._northEast.lng]+"",
                    ""+[mapGeospatialDiv.getBounds()._southWest.lat, mapGeospatialDiv.getBounds()._northEast.lng]+"",
                    ""+[mapGeospatialDiv.getBounds()._southWest.lat, mapGeospatialDiv.getBounds()._southWest.lng]+""];
            ...
            formatted_for_download(incomingFileName){
                let someVar = 'global_attributes:' + '\r\n' +
                    '  data_layer_title: ' + this.title + '\r\n' +
                    '  description: ' + this.description + '\r\n' +
                    '  collection_attached: ' + this.collectionAttached + '\r\n' +
                    ...
                    '  time_period_of_collection: ' + this.time_period_content + '\r\n' +
                    ...
                    '  original_source: ' + this.source1  + '\r\n' +
                    '  current_source: ' + this.source2  + '\r\n' +
        ...
        mounted() {
            this.setupGeospatialLeafletMap();
            //this.invalidSize();
        },
